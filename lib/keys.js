...
exports.renamenx = function (mockInstance, key, newKey, callback) {
  ...
  if (key in mockInstance.storage) {
    if (newKey in mockInstance.storage) {
      ...
      delete mockInstance.storage[key];
      if (mockInstance.storage[newKey].expires) {
        var milli = mockInstance.storage[newKey].expires - new Date();
        var _expire = setTimeout(function() {
          delete mockInstance.storage[newKey];
        }, milli);
        if (_expire.unref) {
          _expire.unref();
        }
        mockInstance.storage[newKey]._expire = _expire;
      }
      result = 1;
    }
  } else {
